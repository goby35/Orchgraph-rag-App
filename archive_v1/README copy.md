...
# Re-embed sang GTE
python re_embedder.py --model bge-m3 --batch-size 50
python re_embedder.py --model gte --batch-size 50
...
## BẢN NÂNG CẤP 
Hệ Sinh Thái Multi-Agent Recruitment — Digital Twin GraphRAGHệ thống kết nối nhân sự thông minh thế hệ mới, ứng dụng kiến trúc Multi-Agent phối hợp với GraphRAG. Hệ thống giải quyết bài toán cốt lõi: "Tuyển dụng đúng người - Chọn đúng việc" thông qua cơ chế phân tầng dữ liệu (Public/Private) trên cùng một Node và giao tiếp hộp đen (Black-box evaluation) giữa các Digital Twin.Kiến trúc tổng quan (Multi-Agent & Data Silos)Plaintext┌──────────────────────────────────────────────────────────────────┐
│                          Streamlit UI                            │
│   Khách (Guest)  ·  Tổ chức (Org)  ·  Cá nhân (Personnel)        │
│   Upload CV/JD  ·  Global Match  ·  Private AI Interview         │
└───────────────────────────┬──────────────────────────────────────┘
                            │
          ┌─────────────────┴─────────────────┐
          ▼                                   ▼
┌──────────────────┐               ┌─────────────────────┐
│  Data Pipeline   │               │   Agentic Engine    │
│  (5 bước MỚI)    │               │                     │
│                  │               │ 1. Master Agent     │
│ Parse → Clean    │               │    (Global Search)  │
│ → LLM Classify   │               │ 2. Org Agent        │
│   (Public/Private│               │ 3. Personnel Agent  │
│ → Vectorize      │               │    (Private Chat)   │
└────────┬─────────┘               └──────────┬──────────┘
         │                                    │
         ▼                                    ▼
┌──────────────────────────────────────────────────────────────┐
│                  Neo4j 5.26 Database                         │
│                                                              │
│ [Ngăn Kính] public_*: Summary, Skills, vector_index (Public) │
│ [Ngăn Khóa] private_*: Salary, Tech Secrets (Secured)        │
│                                                              │
│  (Organization)─[:SIMILAR_TO {score}]─>(Personnel)           │
│  (Organization)─[:CONNECTED_TO {status}]─>(Personnel)        │
│                                                              │
│ *Property-based Access Control: Chặn Master Agent đọc Private│
└──────────────────────────────────────────────────────────────┘
Ba thành phần chính (Kiến trúc v2):#Thành phầnMô tả1Agentic UI (app.py)Giao diện điều phối: Tìm kiếm ứng viên, Gửi yêu cầu kết nối, và Khung chat "Digital Twin Interview".2Split-Data Pipeline (pipeline/)Chuỗi 5 bước: Parse → Clean → Classify (Phân tách Public/Private bằng LLM) → Multi-Vectorize → Upsert Neo4j.3Multi-Agent EngineMaster Agent tìm kiếm bằng Hybrid Search trên public_*. Các Sub-Agent tự động thẩm định bằng dữ liệu private_* sau khi người dùng Accept.Pipeline xử lý dữ liệu (Split-Ingestion)Pipeline được tinh chỉnh để áp dụng chiến thuật "Một Node - Hai Ngăn", bảo vệ dữ liệu nhạy cảm ngay từ cửa ngõ:Plaintext File gốc (CV/JD)
       │
       ▼
 ┌─────────────┐
 │  1. PARSE   │  Docling (primary) → Nutrient.io API (fallback)
 └──────┬──────┘
        ▼
 ┌─────────────┐
 │  2. CLEAN   │  Chuẩn hóa Text, Unicode NFC
 └──────┬──────┘
        ▼
 ┌──────────────┐
 │ 3. CLASSIFY  │  LLM (Claude/GPT-4o) đóng vai trò Data Architect.
 │  (LLM Ext)   │  Phân loại text vào 2 nhóm: Public JSON & Private JSON.
 └──────┬───────┘
        ▼
 ┌───────────────┐
 │ 4. VECTORIZE  │  Multi-embedding (PhoBERT / BGE-M3 / OpenAI).
 │               │  Vector hóa riêng biệt mảng Public và Private.
 └──────┬────────┘
        ▼
 ┌───────────────┐
 │  5. INGEST    │  Upsert vào 1 Node duy nhất trong Neo4j.
 │   (Neo4j)     │  Cập nhật thuộc tính public_* và private_*.
 └───────────────┘
Graph Schema (Neo4j)Node types (Thực thể chính)LabelMô tảKey PropertyPersonnelHồ sơ cá nhân ứng viênid (UNIQUE)OrganizationHồ sơ công ty, dự ánid (UNIQUE)Relationships (Trạng thái kết nối)Luồng kết nối được quản lý bằng các đường nối có hướng, đại diện cho State Machine của quy trình tuyển dụng:Cypher// Bước 1: Thuật toán AI gợi ý (System-generated)
(o:Organization)-[:SIMILAR_TO {score: 0.85, model: "phobert_v2"}]->(p:Personnel)

// Bước 2: Công ty chủ động mời (User-generated)
(o:Organization)-[:CONNECTED_TO {status: "pending"}]->(p:Personnel)

// Bước 3: Ứng viên đồng ý -> Mở khóa Private Data
(o:Organization)-[:CONNECTED_TO {status: "accepted"}]->(p:Personnel)
Cấu trúc "Một Node - Hai Ngăn" (Personnel Node)Toàn bộ thông tin được nén vào các Properties của Node để tăng tốc độ truy xuất và dễ dàng phân quyền:JSON{
  "id": "p_001",
  "public_name": "Phan Minh Đức",
  "public_summary": "Kỹ sư phần mềm cao cấp...",
  "public_skills": ["Neo4j", "GraphRAG", "Blockchain"],
  "public_embeddings_phobert": [0.12, -0.45, ...], 
  
  "private_data_blob": "{'salary': 3000, 'blacklist': ['Cty A']}",
  "private_sentences": ["Logic Escrow dùng Smart Contract..."],
  "private_embeddings_phobert": [0.88, 0.11, ...]
}
Cơ chế Bảo mật (Neo4j Access Control)Sử dụng tính năng phân quyền cấp độ thuộc tính (Property-based Privilege) để đảm bảo Master Agent không thể rò rỉ dữ liệu Private.Cypher-- Cấp quyền cho Master Agent (Chỉ đọc Public)
CREATE ROLE MasterAgentRole;
GRANT TRAVERSE ON GRAPH * TO MasterAgentRole;
GRANT READ {public_*} ON GRAPH * TO MasterAgentRole;
DENY READ {private_*} ON GRAPH * TO MasterAgentRole;

-- Tạo Vector Index chỉ trên trường Public
